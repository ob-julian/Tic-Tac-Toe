...
function onlineFunction() {
    ...
    socket.on('erro', function(){showAlert('Error:<br>Bitte starte dein Spiel neu', 'hid');err();});
    ...
    socket.on('serverRestart', async function(server){
        if((serverId != server) && (serverId != undefined)){
            showAlert('Bitte starte dein Spiel neu.<br>Es gab einen Server Neustart', 'hid');err();
...
function nochnichtgespielt(){
    ...
    showAlert('Sieht so aus als würdest du das erste mal diesen Modus spielen. <br>Willst du ein Tutorial?<br><div  align=\'left\' style=\'float:left;\'><button onclick=\'ja(), experimentalTutorialfals()\'>Ja</button></div><div  align=\'right\' style=\'margin-bottom: -18px;\'><button onclick=\'nein(), experimentalTutorialfals()\'>Nein</button></div>', 'hid');
}
...
async function help() {
    document.getElementById('help').style.display = 'none';
    showAlert('<b>Hilfe</b><br><br>Willst du ein Tutorial zu diesem Spielmodus?<br><br><div  align=\'left\' style=\'float:left;\'><button onclick=\'ja()\'>Ja</button></div><div  align=\'right\' style=\'margin-bottom: -18px;\'><button onclick=\'sp()\'>Nein</button></div>', 'hid');
}
